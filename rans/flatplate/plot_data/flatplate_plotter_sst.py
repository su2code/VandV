...
# \file flatplate_plotter_sst.py
#  \brief Python script for post-processing the turbulent 2D flat plate (SST) solution.
...
h4 = mlab.csv2rec("history_035x025_sst.csv", comments='#', skiprows=0, checkrows=0)
h3 = mlab.csv2rec("history_069x049_sst.csv", comments='#', skiprows=0, checkrows=0)
h2 = mlab.csv2rec("history_137x097_sst.csv", comments='#', skiprows=0, checkrows=0)
h1 = mlab.csv2rec("history_273x193_sst.csv", comments='#', skiprows=0, checkrows=0)
h0 = mlab.csv2rec("history_545x385_sst.csv", comments='#', skiprows=0, checkrows=0)
...
plt.title(r'Residual Convergence for a Turbulent Flat Plate (SST)', fontsize=12)
...
plt.savefig('flatplate_residual_convergence_sst.%s'%(imgfrm),format=imgfrm,dpi=my_dpi*4)
...
plt.ylim([0.0025,0.0029])
plt.xlim([0.0,0.03])
plt.xlabel(r'$h = \sqrt{1/N}$', fontsize=20)
plt.ylabel(r'$C_d$', fontsize=20)
plt.title(r'Drag Grid Convergence Comparison for a Turbulent Flat Plate (SST)', fontsize=12)
...
plt.gcf().subplots_adjust(left=0.18)
plt.gcf().subplots_adjust(bottom=0.13)
plt.savefig('flatplate_cd_gridconv_sst.%s'%(imgfrm),format=imgfrm,dpi=my_dpi*4)

# Compute the skin friction coefficient for plate location x = 0.97
su2_cf = zeros(5)
cf4_data = mlab.csv2rec("surface_035x025_sst.csv", comments='#', skiprows=0, checkrows=0)
cf3_data = mlab.csv2rec("surface_069x049_sst.csv", comments='#', skiprows=0, checkrows=0)
cf2_data = mlab.csv2rec("surface_137x097_sst.csv", comments='#', skiprows=0, checkrows=0)
cf1_data = mlab.csv2rec("surface_273x193_sst.csv", comments='#', skiprows=0, checkrows=0)
cf0_data = mlab.csv2rec("surface_545x385_sst.csv", comments='#', skiprows=0, checkrows=0)
...
plt.title(r'Skin Friction Grid Convergence Comparison for a Turbulent Flat Plate (SST)', fontsize=12)
plt.legend([r'CFL3D',r'FUN3D',r'SU2'],loc=3,fontsize=16)
for label in plt.gca().get_xticklabels() + plt.gca().get_yticklabels():
  label.set_fontsize(13)
plt.grid('on')
plt.gcf().subplots_adjust(left=0.18)
plt.gcf().subplots_adjust(bottom=0.13)
plt.savefig('flatplate_cf_0p97_gridconv_sst.%s'%(imgfrm),format=imgfrm,dpi=my_dpi*4)
...
cfl3d_cf = mlab.csv2rec("cfl3d_cf_545x385_sst.csv", comments='#', skiprows=0, checkrows=0)
fun3d_cf = mlab.csv2rec("fun3d_cf_545x385_sst.csv", comments='#', skiprows=0, checkrows=0)
...
plt.title(r'Skin Friction Coefficient on 545x385 Flat Plate Grid (SST)', fontsize=12)
...
plt.gcf().subplots_adjust(left=0.18)
plt.gcf().subplots_adjust(bottom=0.13)
plt.savefig('flatplate_cf_profile_sst.%s'%(imgfrm),format=imgfrm,dpi=my_dpi*4)
